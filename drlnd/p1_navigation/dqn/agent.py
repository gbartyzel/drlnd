...
class Agent(object):
    def __init__(self, o_dim, u_dim, lrate, tau, gamma, eps_decay,
                 update_freq, buffer_size, batch_size):
        ...
        self._main_checkpoint_path = "./models/checkpoint_main.pth"
        self._target_checkpoint_path = "./models/checkpoint_target.pth"

        self._dqn_main = QNetworkDense(o_dim, u_dim).to(self._device)
        self._dqn_target = QNetworkDense(o_dim, u_dim).to(self._device)
    ...
    def save_model(self):
        if not os.path.isdir("models"):
            os.mkdir("models")
        torch.save(self._dqn_main.state_dict(), self._main_checkpoint_path)
        torch.save(self._dqn_target.state_dict(), self._target_checkpoint_path)

    def load_model(self):
        if os.path.exists(self._main_checkpoint_path):
            self._dqn_main.load_state_dict(torch.load(self._main_checkpoint_path))
        if os.path.exists(self._target_checkpoint_path):
            self._dqn_target.load_state_dict(torch.load(self._target_checkpoint_path))
